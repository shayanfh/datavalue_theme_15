...
frappe.call({
        method: "datavalue_theme_15.datavalue_theme_15.doctype.theme_settings.theme_settings.get_theme_settings",
        callback: function (r) {
            if (!r.message) return;

            let { card_image, powered_by, link, login_desc, login_head } = r.message;

            if (link && !link.startsWith("http")) {
                link = "https://" + link;
            }

            powered_by = powered_by || "Your Brand";
            link = link || "#";

            $('[data-action="login-api"]').html(`
                ${card_image ? `<img class="app-login-bg" src="${card_image}">` : ``}
                <div class="text-center login-copyrights">
                    Powered by
                    <a href="${link}" target="_blank">${powered_by}</a>
                </div>
            `);

            if (login_desc) {
                $('.text-secondary.font-size-md').text(login_desc);
            }
            if (login_head) {
                $('.login_head').text(login_head);
            }
        }
    });
